...
/**
 * 1. there are dependency between indexes.
 * */
...
public class AdLevelDataHandler {
    ...
    public static void handleLevel3(AdUnitTable unitTable, OpType type){
        ...
        if(adPlanObject == null){
            log.error("handleLevel3 found AdPlanObject err: {}",
            unitTable.getPlanId());
            return;
        }
    ...
    public static void handleLevel3(AdCreativeUnitTable creativeUnitTable, OpType type){
        if(type == OpType.UPDATE){
            log.error("CreativeUnitIndex not supporting update!");
            return;
        }
    ...
    public  static void handleLevel4(AdUnitDistrictTable unitDistrictTable, OpType type){
        if(type == OpType.UPDATE){
            log.error("district not supporting update!");
            return;
        }
        ...
        if(null == unitObject){
            log.error("AdUnitDistrict index error : {}",
    ...
    public static void handleLevel4(AdUnitItTable unitItTable, OpType type){
        if(type == OpType.UPDATE){
            log.error("it not supporting update!");
            return;
        }
    ...
    public static void handleLevel4(AdUnitKeywordTable unitKeywordTable, OpType type){
        if(type == OpType.UPDATE){
            log.error("keyword not supporting update!");
            return;
        }
